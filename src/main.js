...
const ui = createUI({
  ...
  onOpenLeaderboard: () => ui.openBoard(board.entries(), board.getName()),
...
// Fake presence pill drifting gently.
let presence = 9 + Math.floor(Math.random() * 13);
ui.setPresence(presence);
setInterval(() => {
  presence = Math.max(6, Math.min(30, presence + (Math.random() < 0.5 ? -1 : 1)));
  ui.setPresence(presence);
}, 6000);
